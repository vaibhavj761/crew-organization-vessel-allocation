          ...
          <details className="ops-editor" key={director.id} open>
            ...
            <div className="ops-editor-body">
              ...
              <div className="subsection-heading spaced">
                ...
                <button className="mini-add" onClick={() => dispatch({ type: 'addOperationsManager', value: { id: id(), crewDirectorId: director.id, sortOrder: directorOps.length + 1, person: person('OPERATIONS_MANAGER'), crewManagers: [] } })}><Plus size={13} /> Add</button>
              </div>
              ...
              <button className="button ghost danger-text" onClick={() => confirm('Delete this Crew Director? Operations Manager hierarchy under it must already be empty.') && dispatch({ type: 'deleteCrewDirector', id: director.id })}><Trash2 size={13} /> Delete Crew Director</button>
            </div>
          </details>
        )
      })}
      {!data.crewDirectors.length && <div className="editor-empty">No Crew Directors added yet.</div>}
      <button className="button add-team" onClick={addDirector}><Plus size={14} /> Add Crew Director</button>
    </div>
  )
}
  ...
  return (
    <details className="nested-editor" open>
      ...
      <div className="nested-form">
        ...
        <div className="subsection-heading spaced">
          <h4>Crew Managers</h4>
          <button className="mini-add" onClick={() => dispatch({ type: 'addCrewManager', operationsManagerId: op.id, value: { id: id(), sortOrder: op.crewManagers.length + 1, person: person('CREW_MANAGER'), assistants: [], vesselIds: [] } })}><Plus size={13} /> Add</button>
        </div>
        ...
        <button className="button ghost danger-text" onClick={() => confirm(`Delete Operations Manager ${op.person.name}? Crew Manager vessel assignments must already be cleared.`) && dispatch({ type: 'deleteOperationsManager', id: op.id })}><Trash2 size={13} /> Delete Operations Manager</button>
      </div>
    </details>
  )
}
  ...
  return (
    <details className="nested-editor">
      ...
      <div className="nested-form">
        ...
        <div className="subsection-heading spaced">
          <h4>Assistants</h4>
          <button className="mini-add" onClick={addAssistant}><Plus size={13}/> Add</button>
        </div>
        {cm.assistants.map(a=><div className="assistant-editor-row" key={a.id}><TextField label="Name" value={a.name} onChange={name=>dispatch({type:'updateAssistant',crewManagerId:cm.id,value:{...a,name}})}/><TextField label="Designation" value={a.designation} onChange={designation=>dispatch({type:'updateAssistant',crewManagerId:cm.id,value:{...a,designation}})}/><button className="tiny-icon danger-text" onClick={()=>confirm('Delete this assistant? Vessel responsibility links will be cleared.')&&dispatch({type:'deleteAssistant',crewManagerId:cm.id,assistantId:a.id})}><Trash2 size={13}/></button></div>)}
        {!cm.assistants.length && <div className="editor-empty">No assistants added yet.</div>}
        <button className="button ghost danger-text" onClick={()=>confirm('Delete this Crew Manager? Vessel assignments will be cleared.')&&dispatch({type:'deleteCrewManager',operationsManagerId:op.id,id:cm.id})}><Trash2 size={13}/> Delete Crew Manager</button>
      </div>
    </details>
  )
}
