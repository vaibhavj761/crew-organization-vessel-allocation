...
const ops = (id: string, crewDirectorId: string, name: string, designation: string, sortOrder: number, crewManagers: CrewManagerNode[]): OperationsManagerNode => ({ id: `ops-node-${id}`, crewDirectorId, sortOrder, person: person(id, name, designation, 'OPERATIONS_MANAGER'), crewManagers })
...
export const sampleData: ChartData = {
  ...
  operationsManagers: [
    ...
    ops('ops-2', directorTwo.id, 'Priya Nair', 'Fleet Operations Lead', 2, [elena]),
  ],
  vessels: [
