...
import { parseLocalAiInstructions } from './localParser.js'
...
export type AiProviderErrorCategory = 'missing_key' | 'invalid_key' | 'model_not_found' | 'rate_limit' | 'quota' | 'network' | 'invalid_response'
...
export function categorizeProviderError(error: unknown): { category: AiProviderErrorCategory; message: string } {
  ...
  if (lower.includes('quota') || lower.includes('insufficient_quota') || lower.includes('credit')) return { category: 'quota', message }
  if (lower.includes('rate limit') || lower.includes('rate_limit_exceeded') || lower.includes('too many requests')) return { category: 'rate_limit', message }
...
export async function runProviderBeforeFallback(
  providerCall: () => Promise<AiStructuredAction[]>,
  fallbackCall: () => AiStructuredAction[] | null,
) {
  try {
    const providerAction = await providerCall()
    return { providerAction, providerError: null, fallbackAction: fallbackCall() }
  } catch (providerError) {
    return { providerAction: null, providerError, fallbackAction: fallbackCall() }
  }
}

export async function interpretAiPrompt(prompt: string, reference: AiReferenceData): Promise<AiInterpretResult | null> {
  if (env.AI_PROVIDER === 'mock') {
    const actions = await interpretWithMock(prompt, reference)
    return { actions, providerUsed: 'mock', fallbackUsed: false, fallbackReason: null }
  }
  if (env.AI_PROVIDER === 'none') return null
  if (!isAiProviderConfigured()) return null

  const attempt = await runProviderBeforeFallback(
    () => interpretWithSelectedProvider(prompt, reference),
    () => parseLocalAiInstructions(prompt),
  )

  if (!attempt.providerError && attempt.providerAction) {
    const aiActions = attempt.providerAction
    clearAiProviderError()
    const localFallbacks = attempt.fallbackAction
    const corrected = aiActions.length === 1 && localFallbacks?.length === 1
      ? applyBackendSanityCorrection(prompt, aiActions[0], localFallbacks[0])
      : null
    if (corrected) return {
      actions: [{
        ...corrected,
    ...
    if (aiActions.every((item) => item.action === 'unsupported') && localFallbacks?.every((item) => item.action !== 'unsupported')) return {
      actions: localFallbacks.map((item) => ({ ...item, warnings: [...item.warnings, 'AI provider could not confidently interpret this; a deterministic local command fallback was used.'] })),
      providerUsed: env.AI_PROVIDER,
      fallbackUsed: true,
      fallbackReason: 'Provider returned unsupported for an obvious deterministic command.',
    }
    return { actions: aiActions, providerUsed: env.AI_PROVIDER, fallbackUsed: false, fallbackReason: null }
  } else {
    const providerError = categorizeProviderError(attempt.providerError)
    recordAiProviderError(providerError.category, providerError.message)
    const localFallbacks = attempt.fallbackAction
    if (localFallbacks?.length && localFallbacks.every((item) => item.action !== 'unsupported')) return {
      actions: localFallbacks.map((item) => ({ ...item, warnings: [...item.warnings, 'Real AI provider was unavailable. This preview was generated by local fallback and may need clearer wording.'] })),
      providerUsed: env.AI_PROVIDER,
      fallbackUsed: true,
      fallbackReason: providerError.message,
    }
    throw attempt.providerError
  }
}
