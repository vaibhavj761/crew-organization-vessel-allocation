...
export async function interpretAiPrompt(prompt: string, reference: AiReferenceData): Promise<AiInterpretResult | null> {
  ...
  const attempt = await runProviderBeforeFallback(
    ...
    () => parseLocalAiInstruction(prompt),
  )

  if (!attempt.providerError && attempt.providerAction) {
    const aiAction = attempt.providerAction
    clearAiProviderError()
    const localFallback = attempt.fallbackAction
    ...
    const localFallback = attempt.fallbackAction
    if (canUseLocalFallback(attempt.providerError, localFallback)) return {
      action: {
        ...localFallback!,
